...
if(!OPEN_BROWSER.match(new RegExp(/(^|\W)true($|\W)|(^|\W)false($|\W)/))) {
...
if(!OPTIMIZE.match(new RegExp(/(^|\W)true($|\W)|(^|\W)false($|\W)/))) {
...
const proxyApiPrefix = 'dev/'; // The element in the URL which differentiate between API request and static file request
